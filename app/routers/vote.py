...
def vote(vote: schemas.Vote, db: Session = Depends(database.get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    ...
    print(f"Vote Direction: {vote.direction}")
    print(f"Found Vote: {found_vote}")
    ...
    if vote.direction == 1:
        ...
        if not found_vote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"No vote on post {vote.post_id} by user {current_user.id} exists")
